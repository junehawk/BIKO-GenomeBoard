...
**Version**: v2.5.3 (2026-04-22) · **Audience**: 변이 분류 결과를 검토·공유받는 연구자와 임상의
...
|---|---|---|---|
| **PVS1** | Very Strong | Pathogenic | Null variant (nonsense / frameshift / canonical ±1,2 splice / initiation codon / single-exon or multi-exon deletion)에서 **LoF가 해당 질환의 알려진 disease mechanism인 gene** (SVI 2018 PVS1 decision tree 준수) |
| **PS1-4** | Strong | Pathogenic | PS1 동일 aa 변화가 이미 pathogenic, PS2 de novo **confirmed** (parent biological relationship 확인 + phenotype specificity), PS3 well-established functional study, PS4 case-control OR significant |
| **PM1-6** | Moderate | Pathogenic | PM1 **benign variation 없는 mutational hotspot 또는 critical/well-established functional domain**, PM2 rare (gene-disease-specific cutoff 미설정 시 BIKO default), PM3 AR gene에서 trans pathogenic, PM4 protein length change (비-repeat 영역), PM5 동일 residue 다른 missense가 pathogenic, PM6 de novo **assumed** (parentage 미확인) |
| **PP1-5** | Supporting | Pathogenic | PP1 cosegregation, PP2 **benign missense variation 적고 missense가 주된 질병 기전인 gene의 missense**, PP3 in silico (REVEL/SpliceAI 등), PP4 phenotype specific to gene, PP5 reputable source (ClinVar Pathogenic) *⁰* |
| **BA1** | Stand-alone | Benign | AF > 5% (ClinGen BA1 Exception list 예외 변이 제외) |
| **BS1-4** | Strong | Benign | BS1 disease prevalence 대비 AF 초과, BS2 **healthy adult에서 full penetrance early-onset 기대되는 AD/XL 변이 관찰 또는 AR 변이의 healthy homozygote**, BS3 functional, BS4 non-segregation |
| **BP1-7** | Supporting | Benign | BP1 truncating mechanism 유전자의 missense, BP2 trans pathogenic, BP4 in silico benign, BP6 reputable source (ClinVar Benign) *⁰*, BP7 synonymous without splice impact |

*⁰ ClinGen SVI 2018은 PP5/BP6 사용을 **deprecated** 권고합니다 (PS1/PM5/population/function/segregation evidence와의 **double-counting** 위험 때문). **v2.5.3 (2026-04-22)에서 scoring 제외 landed**: `scripts/classification/acmg_engine.py::_count_by_strength`가 PP5/BP6를 기본적으로 무시합니다 (`counts["pp"]` / `counts["bp"]`에 반영되지 않음). **Annotation(리포트 evidence_codes 표시)은 유지** — ClinVar DB/InterVar emission 경로 (`scripts/enrichment/query_clinvar.py`, `scripts/storage/query_local_clinvar.py`, `scripts/annotation/parse_intervar.py`)는 수정하지 않아, 리포트에 "ClinVar Pathogenic에서 PP5가 함께 참조됨"이 계속 표시됩니다. 즉 **evidence-as-signal (annotation)은 유지, evidence-as-score (counter)는 제외**. §1.4 A4 4-조건 gate는 PP5/BP6에 의존하지 않으므로 영향 없음. Back-compat 벤치마크가 필요하면 `config.yaml::acmg.use_pp5_bp6: true`로 v2.5.2 이전 동작 복원 가능.*

**Modified-strength evidence (BIKO 구현)**: ClinGen SVI는 de novo PS2/PM6의 경우 confirmed/assumed 구분 + phenotype specificity + observation count를 합산하는 **point-based** 체계를 권고 (PS2/PM6 v1.1). **BIKO는 현재 이를 단순화**해 `PS2_Moderate` (confirmed de novo 1건) / `PM6_Supporting` (assumed de novo 1건)로 고정 발화합니다 — 이는 BIKO simplification이며 ClinGen point system을 전부 구현한 것이 아닙니다. 추가 de novo 관측 또는 phenotype specificity가 높은 경우 수동 upgrade 검토 필요.
...
`data/acmg_rules.json`은 **Richards 2015 원문 조합표**를 그대로 수록합니다 (point/odds 기반 Bayesian 엔진 아님). Modified-strength evidence (예: PS2_Moderate, PP3_Strong, PM2_Supporting)는 `scripts/classification/acmg_engine.py::_count_by_strength`가 suffix를 해석해 **표기 강도에 해당하는 카운터에 집계**하도록 v2.5.2에서 수정되었습니다:

- `PP3_Strong` → `counts["ps"]` (Strong으로 집계)
- `PS2_Moderate`, `PP3_Moderate` → `counts["pm"]` (Moderate로 집계)
- `PM6_Supporting`, `PM2_Supporting` → `counts["pp"]` (Supporting로 집계)

예: `PS2_Moderate + PM2_Supporting + PP3_Strong` 3개 근거 → `counts = {ps: 1, pm: 1, pp: 1}` → `1 PS + 1 PM + 1 PP`? Likely Pathogenic 조합에는 매치되지 않지만, PP3_Strong의 ps 카운트 1 + PS2_Moderate의 pm 카운트 1 → `1 PS + 1 PM` = **LP** 조합 성립. 즉 modified-strength는 문자열 suffix → 카운터 집계 → 원문 조합 테이블 매치 순서로 작동합니다.
...
**In silico fallback** (BIKO internal, ClinGen SVI 2022 권고 목록 밖): REVEL이 없는 경우 CADD Phred ≥ 25 + AlphaMissense likely_pathogenic 조합을 `PP3` 대용으로 사용 (`scripts/classification/in_silico.py`). **AlphaMissense는 Pejaver 2022 calibration 대상 도구 목록에 포함되지 않으며**, 이 fallback은 BIKO 고유 internal policy입니다 — SVI 공식 권고 아님. REVEL이 있으면 이 fallback은 사용되지 않습니다.
...
*⁵ ClinGen SVI 2018은 PP5/BP6를 deprecated로 권고합니다 (ClinVar entry를 별도 evidence로 인정하지 않음). **v2.5.3 기준 BIKO는 PP5/BP6 발화는 유지하되 scoring에서 제외**합니다 (footnote ⁰ 참조). 리포트의 `evidence_codes` 열에는 계속 표시되지만, Pathogenic / Likely Pathogenic / Likely Benign 조합 판정에는 카운트되지 않습니다. ClinVar "conflicting" 항목에 대한 §1.4 A4 gate는 PP5/BP6와 독립적으로 PM1 + PM5 만 요구하므로 영향 없음.*
...
**Oncogenicity vs Actionability — BIKO 정책 명시**: BIKO의 cancer mode Tiering은 **actionability (predictive/diagnostic/prognostic clinical significance)** 에 초점을 둔 internal reporting policy입니다. 2022년 ClinGen/CGC/VICC가 별도로 발표한 **somatic oncogenicity classification SOP** (Oncogenic / Likely Oncogenic / VUS / Likely Benign / Benign)는 BIKO에서 **별도 필드로 표기하지 않으며**, 리포트의 "Pathogenic/Likely Pathogenic"는 germline ACMG 엔진 결과가 cancer context에서 AMP tiering 계산용으로 활용됨을 의미합니다. 정식 somatic oncogenicity annotation이 필요한 경우 별도 도구 (CancerVar 등) 검토 필요.

**CIViC Evidence Level** (공식 정의, Griffith et al. 2017):
- **A** = Validated association (clinical-grade evidence)
- **B** = Clinical evidence (case series, clinical trial data)
- **C** = Case study (single or few case reports)
- **D** = Preclinical (in vitro / animal model)
- **E** = Inferential association (indirect evidence)

> ⚠ 이전 버전(v2.5.1까지) 문서의 "A=FDA-approved, B=Professional guidelines" 설명은 AMP/OncoKB식 해석이 섞인 **부정확한 표기**였습니다 (v2.5.2 수정). CIViC 원문은 evidence **type** (validated/clinical/case/preclinical)으로 구분하며, FDA 승인 여부와 직접 대응하지 않습니다.

**OncoKB Level** (therapeutic level, tumor-type-specific alteration 기준):
- **Level 1** = FDA-recognized biomarker + FDA-approved drug for that indication
- **Level 2** = Standard care biomarker for that indication
- **Level 3A/3B** = Compelling clinical evidence, off-label / cross-indication
- **Level 4** = Compelling biological evidence / preclinical

> ⚠ **OncoKB는 gene 전체가 아니라 tumor-type-specific individual alteration에 level을 할당**하는 것이 공식 원칙입니다. BIKO의 "OncoKB gene Level 1/2 → Tier I" 결정 규칙 (§2.2 #7)은 구현 단순화로, **alteration-specific match 우선 + OncoKB 'Oncogenic Mutations' umbrella 예외 허용**으로 작동합니다. 임상 해석 시 OncoKB 원 entry를 교차 확인 권장.
...
- **BIKO internal reporting bucket** (공식 임상/규제 cutoff 아님):
  - Low: < 10 mut/Mb
  - Intermediate: 10–20
  - High: ≥ 20 mut/Mb
- 패널 크기 `panel_size_mb` 기준 (WGS 기본 33 Mb)

> ⚠ **참고**: FDA pembrolizumab tissue-agnostic 적응증은 **TMB-H ≥ 10 mut/Mb** (FDA-approved test 기준)를 사용합니다. AMP/CAP/SITC 2024 권고는 TMB **validation + reporting standardization**에 초점이 있으며, 보편적 Low/Intermediate/High bin을 고정하지 **않습니다**. 위 20 mut/Mb threshold는 reporting 목적의 BIKO internal policy이며, **실제 임상 의사결정 시 raw TMB 값 + assay/platform + FDA/ESMO 가이드라인 기준**을 동시에 확인하십시오. 리포트에는 raw TMB 값 + panel size + BIKO bucket이 모두 표시됩니다.
...
**Homozygote count** (KOVA 전용 필드): AR disease 검토 시 KOVA에 homozygote가 관측되면 BS2 **후보 근거**로 참고 가능. 단, KOVA 공개 설명에 따르면 cohort에 cancer patient의 normal tissue, rare-disease patient의 healthy parents, healthy volunteers가 혼재되어 있으므로 **단순 "healthy homozygote observed"로 BS2 자동 발화는 지양**됩니다. BS2 적용 시 (a) 해당 variant의 disease가 early-onset full penetrance 전제이고, (b) KOVA subject가 연령/표현형상 healthy임이 확인되어야 합니다 — 현재는 **수동 검토 단계**이며 v2.6에서 KOVA metadata 활용한 반자동 BS2 검토 기능 예정.
...
A. AR 질환 검토에서 BS2 **후보 근거**로 참고 가능하지만 자동 발화는 하지 않습니다. KOVA cohort는 순수 healthy population이 아니라 cancer normal tissue + rare-disease parents + healthy volunteer 혼재 — 따라서 homozygote가 있다고 바로 "healthy"로 단정할 수 없음. BS2 적용 전 (1) 해당 disease가 early-onset + full penetrance인지, (2) 해당 homozygote subject의 age/phenotype이 healthy로 명백한지를 별도 확인해야 합니다. §6 참조.
...
**Q. PP5/BP6는 어떻게 다루나요? ClinGen SVI 2018이 deprecated라고 했는데.**
A. **v2.5.3에서 scoring 제외 반영 완료**. `_count_by_strength`가 PP5/BP6를 기본적으로 카운트하지 않으므로 double-counting 위험이 해소됐습니다. 다만 ClinVar entry의 존재 자체는 정보 가치가 있으므로 **annotation(evidence_codes 표시)은 유지**합니다 — 리포트 독자가 "ClinVar에 Pathogenic entry가 있었지만 SVI 2018에 따라 별도 scoring하지 않음"을 확인할 수 있도록. §1.4 A4 gate는 PM1 + PM5만 요구하므로 PP5/BP6 제외와 무관하게 작동합니다.
...
*문의: BIKO GenomeBoard 팀 (juneh@kisti.re.kr). 이 문서는 v2.5.3 (2026-04-22) 기준이며, 가이드라인 업데이트 시 재개정됩니다. v2.5.3 개정: ACMG evidence 코드(PVS1/PP2/PM1/BS2) 정확 표현, modified-strength × 조합표 연결 명시, CIViC/OncoKB 공식 정의 교정, oncogenicity vs actionability 분리, TMB internal bucket 명시, KOVA homozygote BS2 보수적 적용. v2.5.3 maintenance note: ClinGen SVI WG는 2025-04 retire되었으며 현재 관리창구는 ClinGen Variant Classification Guidance page (clinicalgenome.org).*
