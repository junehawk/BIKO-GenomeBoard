...
## Rare-disease primary diagnosis — 필수 규칙 (MANDATORY)

**케이스 정보에 `PRIMARY DIAGNOSIS CANDIDATES (phenotype-matched)` 섹션이 있다면**:
1. `primary_diagnosis`는 **반드시** 그 섹션에 나열된 변이/유전자 중 하나로 지정하시오.
   다른 유전자를 primary_diagnosis에 쓰는 것은 금지됩니다.
2. 그 섹션에 없는 Pathogenic 또는 Likely-Pathogenic 변이(예: 제출된 HPO 표현형과
   무관한 유전자의 CFTR 같은 pathogenic 변이)는 **반드시** `key_findings` 또는
   `follow_up`에 `incidental finding` 또는 `secondary finding: phenotype과 무관한
   {gene} pathogenic 변이 — carrier status 가능성`으로 분리 보고하시오.
3. LLM이 학습한 gene-disease 연관 지식(예: "CFTR → Cystic Fibrosis")을
   primary_diagnosis로 삼으려 하지 마시오. 제출된 HPO 표현형이 우선입니다.

**PRIMARY DIAGNOSIS CANDIDATES 섹션이 없거나 비어있는 경우**에만:
- 제공된 임상 맥락과 ACMG 분류를 종합해 primary_diagnosis를 제안할 수 있습니다.

근거: VUS이더라도 표현형 일치 유전자의 de novo 변이는 가족 분리분석·기능연구로
...
## Rare-disease primary diagnosis — MANDATORY rules

**If the case briefing contains a `PRIMARY DIAGNOSIS CANDIDATES
(phenotype-matched)` section**:
1. `primary_diagnosis` MUST name a variant/gene from that section.
   Naming any other gene in `primary_diagnosis` is forbidden.
2. Any Pathogenic or Likely-Pathogenic variant NOT in that section (for
   example a CFTR pathogenic variant when the submitted HPO is autism)
   MUST be reported under `key_findings` or `follow_up` as
   `incidental finding` or `secondary finding: phenotype-unrelated
   {gene} pathogenic variant — possibly carrier status`.
3. Do NOT anchor the primary diagnosis on gene-disease associations you
   learned during training (e.g. "CFTR → Cystic Fibrosis"). The submitted
   HPO phenotype takes precedence.

**Only when the PRIMARY DIAGNOSIS CANDIDATES section is absent or empty**
you may propose a `primary_diagnosis` from the broader clinical context
and ACMG classifications.

Rationale: a VUS in a phenotype-concordant gene — especially a de novo
variant — is the top candidate pending family segregation and functional
