...
1. **WGS/WES 결과가 쏟아지지만 한국인 빈도로 필터링된 결과가 필요**합니다 — gnomAD만으로는 한국인 환자에게 덜 정확합니다. KOGO/gene2korea가 발표한 **Korean Variant Archive (KOVA) v7** (43M variants, homozygote counts 포함)을 gnomAD EAS/ALL과 함께 비교해야 BA1/BS1/PM2 판정이 임상에 의미 있습니다.
...
| **한국인 코호트 연구자** | KOVA v7 한국인 빈도(homozygote count 포함)를 리포트에 직접 반영 |
...
flowchart TD
    ...
    C --> D[3 · 한국인 빈도 비교<br/>KOVA v7 · gnomAD EAS · gnomAD ALL<br/>3-tier + Korean enrichment + hom counts]
...
- **한국인 빈도 3-tier 비교 (KOVA v7)** — KOVA v7 (43M variants, homozygote counts) + gnomAD EAS + gnomAD ALL을 동시에 비교해 BA1/BS1/PM2 판정, Korean enrichment ratio 자동 계산. KOVA의 homozygote count는 상염색체 열성 질환 판정에 특히 중요
...
| 자동 설치 | 수동 설치 필요 |
|---|---|
| ClinVar · gnomAD · CIViC · HPO · Orphanet · GeneReviews · KOVA v7 · cancerhotspots | OMIM genemap2 (계정 필요) · ClinGen(웹 내보내기) |
